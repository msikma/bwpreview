...
const logFeedback = (quiet, filepath, skipExists, skipObs, {imgType, width, height, start, size} = {}) => {
  ...
  if (skipExists || skipObs) {
    return console.log(`Skipping ${skipObs ? 'observer map' : 'existing'} image: ${filepath}`)
  }
...
const generateMapPreview = async (file, gfx, {obsSkip, imgSkip, imgType, imgSize, useInternalName, nameSuffix, outFlatten, outQuiet, pathRel, pathOut = null} = {}) => {
  ...
  const fileRel = pathRel ? path.relative(pathRel, target) : target
  ...
  const [buffer, imgWidth, imgHeight] = await encodeMapImage(imgType, img, width, height, imgSize)
  await ensureDir(path.dirname(target))
  await fs.writeFile(target, buffer, null)
  logFeedback(outQuiet, fileRel, false, false, {imgType, width: imgWidth, height: imgHeight, start, size: Buffer.byteLength(buffer)})
}
